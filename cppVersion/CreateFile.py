...
def handle_existing_file(base_filename):
    ...
    # 如果文件存在且不是以_v数字结尾
    if os.path.exists(base_filename) and not name.endswith(
...
def get_problem_slug_by_test_number(test_number):
    ...
    if response.status_code != 200:
        raise Exception(
            f"Failed to fetch problem list. Status code: {response.status_code}"
        )
...
def get_cpp_template_by_test_number(test_number, cookie):
    ...
    # 提取 C++ 代码模板
    cpp_template = json_data["props"]["pageProps"]["dehydratedState"]["queries"][1][
        "state"
    ]["data"]["question"]["codeSnippets"][0]["code"]
