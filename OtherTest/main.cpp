...
public:
  void Calc(vector<char> &remainCharVec, string &curResult,
    ...
    if (find(remainCharVec.begin(), remainCharVec.end(), '(') !=
      ...
      curResult.push_back('(');
      auto new_end =
          std::remove(remainCharVec.begin(), remainCharVec.end(), '(');
      remainCharVec.erase(new_end, remainCharVec.end());
      unMatchedLeftCount++;
...
public:
  vector<string> generateParenthesis(int n) {
    ...
    std::fill(remainCharVec.begin(), remainCharVec.begin() + n / 2, '(');
    std::fill(remainCharVec.begin() + n / 2, remainCharVec.end(), ')');
